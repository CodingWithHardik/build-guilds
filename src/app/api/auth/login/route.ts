...
export async function POST(request: NextRequest) {
    ...
    if (typeof token === "object" && token !== null && "code" in token && token.code === 202 && "token" in token) {
        ...
        return new Response(JSON.stringify({ token: token.token, isNew: !getData, name: getData?.name || null, email: getData?.email || null, avatar: getData?.avatar || null }), { status: 202 });
    }
}